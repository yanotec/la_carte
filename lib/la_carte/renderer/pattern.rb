    ...
    class Pattern
      ...
      def check_definition_set!
        if not set_definitions?
          ...
          definitions.each do |type, pattern|
            check_definition_reference_in_conditions type, pattern
            pattern.each do |name, definition|
              check_definition_reference_in_content type, name, (definition || {})[:content]
            end
          end
        end
      end
      ...
      def content_tag(tag_name, *args)
        ...
        if block_given?
          ...
          if @__tag_buffer.any?
            if EMPTY_HTML_ELEMENTS.include? tag_name.to_sym
